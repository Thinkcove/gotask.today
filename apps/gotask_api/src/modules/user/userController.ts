...
class UserController extends BaseController {
  ...
  async addUserCertificates(requestHelper: RequestHelper, handler: any) {
    try {
      ...
      const updatedUser = await userService.addCertificates(userId, certificates);
      return this.sendResponse(handler, updatedUser);
    } catch (error) {
      return this.replyError(error);
    }
  ...
  //   try {
  //     const userId = requestHelper.getParam("id");
  //     const { increment } = requestHelper.getPayload();
  //     if (!increment || !increment.date || increment.ctc == null) {
  //       throw new Error("Increment object with date and ctc is required.");
  //     }

  //     const result = await userService.addIncrement(userId, increment);
  //     return this.sendResponse(handler, result);
  //   } catch (error) {
  //     return this.replyError(error);
  //   }
