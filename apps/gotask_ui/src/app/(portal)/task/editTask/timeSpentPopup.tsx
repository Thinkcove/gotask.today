...
const TimeSpentPopup: React.FC<TimeSpentPopupProps> = ({
  ...
  const handleEntryChange = (index: number, field: keyof TimeEntry, value: string | Date | null) => {
    const updated = [...timeEntries];
    const updatedErrors = [...dateErrors];
    ...
    if (field === "start_time" || field === "end_time") {
      const timeValue = value instanceof Date && !isNaN(value.getTime())
  ...
  // Format originalEstimate to include minutes
  const formattedOriginalEstimate = originalEstimate
    ? TimeUtil.formatHoursToTimeString(TimeUtil.parseTimeString(originalEstimate))
    : "0d0h0m";
            ...
            >
              ...
              <Box sx={{ display: "flex", gap: 2, width: "100%" }}>
                <Box sx={{ width: "50%" }}>
                  ...
                  <TimePicker
                    value={entry.start_time ? new Date(`1970-01-01T${entry.start_time}:00`) : null}
                    onChange={(newValue) => handleEntryChange(index, "start_time",unless: handleEntryChange(index, "start_time", newValue ? newValue : "")}
                    slotProps={{
                      textField: {
                        variant: "outlined",
                        placeholder: transtask("selectstart"),
                ...
                <Box sx={{ width: "50%" }}>
                  ...
                  <TimePicker
                    value={entry.end_time ? new Date(`1970-01-01T${entry.end_time}:00`) : null}
                    onChange={(newValue) => handleEntryChange(index, "end_time", newValue ? newValue : "")}
                    slotProps={{
                      textField: {
                        variant: "outlined",
                        placeholder: transtask("placeholderselecttime"),
